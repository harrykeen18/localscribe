...
/// Remote Ollama service that allows HTTP connections to any endpoint
/// NOTE: Shared types (SummaryResult, SummarizationError, OllamaAPIType) are defined in OllamaLocalService.swift
@MainActor
class OllamaRemoteService: ObservableObject, SummarizationProvider {
    static let shared = OllamaRemoteService()

    // SummarizationProvider conformance
    var providerName: String { "ollamaRemote" }
    var displayName: String { "Ollama (Remote/Custom)" }
    ...
    private init() {
        logger.info("OllamaRemoteService initialized (allows remote HTTP connections)", category: .transcription)
    }
    ...
    func generateTitle(from summary: String) async throws -> String {
        logger.info("Generating title with Ollama Remote", category: .transcription)
        ...
        logger.info("Successfully generated title with Ollama Remote", category: .transcription)
        return cleanedTitle
    }
...
// NOTE: Request/Response models (ChatCompletionRequest, ChatMessage, etc.) are defined in OllamaLocalService.swift
